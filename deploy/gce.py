...
class GceInventory(object):
    ...
    def group_instances(self):
        ...
        for node in self.driver.list_nodes():
            ...
            for metavalue in (node.extra['metadata']['items']):
                if metavalue['key'] == "ansible_host_groups":
                    group_names = metavalue['value'].split(",")
                    for group_name in (group_names):
                        if groups.has_key(group_name): groups[group_name].append(name)
                        else: groups[group_name] = [name]
